...
function CustomerAdModal({ show, handleClose, user, onLoadData, onModalDelete, showModalDelete }: ModalShow) {
    ...
    const handleSubmitForm = async (value: any) => {
        ...
        await value.avatar.forEach(async (img: any, index: number) => {
            if (img.file) {
            ...
            if (index === value.avatar.length - 1) {
                ...
                if (value._id) {
                    try {
                        ...
                        console.log({ avatar: images[0], address: JSON.stringify(addressDetail), ...others })
                        await usersService.update(_id,{ avatar: images[0], address: JSON.stringify(addressDetail), ...others })
                        dispatch(showToast({ show: true, text: "Cập nhật khách hàng thành công", type: "success", delay: 1500 }))
                        onLoadData()
                        setLoading(false)
                        handleClose()
                    } catch (err) {
                        dispatch(showToast({ show: true, text: "Cập nhật khách hàng thất bại", type: "error", delay: 1500 }))
                        setLoading(false)
                        handleClose()
                    }
        ...
        <>
            ...
            >
                <Modal.Header closeButton>
                    <h5 className="mb-0">{user?._id === '' ? 'Thêm' : 'Cập Nhật'} Khách Hàng</h5>
                </Modal.Header>
...
export default CustomerAdModal;