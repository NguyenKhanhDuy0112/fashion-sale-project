    ...
    const handleDeleteTrademark = async () => {
        ...
        if (trademark?._id) {
            try {
                ...
                dispatch(showToast({ show: true, text: "Xóa thương hiệu thành công", type: "success", delay: 1500 }))
                setIsLoading(false)
                onLoadData()
            }
            catch (err) {
                dispatch(showToast({ show: true, text: "Xóa thương hiệu thất bại", type: "error", delay: 1500 }))
                setIsLoading(false)
            }
        }
        onModalDelete()
